...
{
	/*
	USES gravanompolig() TO CALCULATE GRAVITY ANOMALY ALONG HORIZONS PROFILE 
			(results in m/s2) 
	USES geoidanompolig() TO CALCULATE GEOID ANOMALY 
			(results in m) 
	...
	*/
	...
	for (i_unit=0; i_unit<numUnits; i_unit++) {
		...
		for (ix=ix_min; ix<ix_max; ix++) {
			gravanom[ix]  += gravanompolig (unit_aux_x, unit_aux_z, np_aux_unit, x0+ix*dx, -alt_measurement[ix], Units[i_unit].density);
			geoidanom[ix] += geoidanompolig(unit_aux_x, unit_aux_z, np_aux_unit, x0+ix*dx, -10e3, Units[i_unit].density);
		}
	}
	...
	if (switch_sea) {
		...
		for (ix=ix_min; ix<ix_max; ix++) {
			gravanom[ix]  += gravanompolig (unit_aux_x, unit_aux_z, np_aux_unit, x0+ix*dx, -alt_measurement[ix], denswater);
			geoidanom[ix] += geoidanompolig(unit_aux_x, unit_aux_z, np_aux_unit, x0+ix*dx, -10e3, denswater);
		}
	}
	...
	for (ix=ix_min; ix<ix_max; ix++) {
		gravanom[ix]  += gravanompolig (unit_aux_x, unit_aux_z, np_aux_unit, x0+ix*dx, -alt_measurement[ix], denscrust);
		aux=geoidanompolig(unit_aux_x, unit_aux_z, np_aux_unit, x0+ix*dx, -10e3, denscrust);
		geoidanom[ix] += aux;
	}
	...
	for (ix=ix_min; ix<ix_max; ix++) {
		aux=gravanompolig (unit_aux_x, unit_aux_z, np_aux_unit, x0+ix*dx, -alt_measurement[ix], densmantle);
		gravanom[ix]  += aux;
		geoidanom[ix] += geoidanompolig(unit_aux_x, unit_aux_z, np_aux_unit, x0+ix*dx, -10e3, densmantle);
	}
	...
	for (ix=ix_min; ix<ix_max; ix++) {
		aux = gravanompolig (unit_aux_x, unit_aux_z, np_aux_unit, x0+ix*dx, -alt_measurement[ix], denscrust);
		gravanom[ix]  -= aux;
		geoidanom[ix] -= geoidanompolig(unit_aux_x, unit_aux_z, np_aux_unit, x0+ix*dx, -10e3, denscrust);
	}
...
int inputs(int argc, char **argv)
{
	...
	/*Version of tAo will be matched later in the parameters file *.PRM*/
