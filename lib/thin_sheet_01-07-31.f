...
CC  Com mes petits siguin el Dx i Dy -> mes petit ha de ser el Dt
...
!       DO 132 kixy=1,nn
          kixy=ix+1+iy*(n+1)
	  thich_layer=ZASTH
	  IF(elevation(kixy).GT.0.D0) thich_layer=ZASTH+elevation(kixy)
	  sed=sediment(kixy)
	  hm=GL(kixy)-s(kixy)-sed
	  ha=ZASTH-GL(kixy)+elevation(kixy)
     ...
     +			(g*RHOAST*ha*ha/2.D0)
     	  average_Pressure(kixy)=(Fw+Fsed+Fc+Fm+Fa)/thich_layer
...
!	      PRINT*,ix,iy,average_Pressure(kixy)
!	      PRINT*,visTer(kixy)
 132   CONTINUE
C       IF(TEMPSMa.EQ.0) WRITE(6,364) DP_inf/1.D6, DP_sup/1.D6
C 364   FORMAT(/2X,'Delta Presion limits: ',F11.2,' MPa/m < DPresion <',
C     +		F11.2,' MPa/m')
      
       WRITE(6,365) rommin,rommax,ZASTH/1.D3,Pmin,Pmax
 365   FORMAT(/2X,'DENSITY OF LITHOSPHERIC MANTLE: minima =',F9.2,
     +		' km/m3,    maxima =',F9.2,' km/m3'/
     +  2X,'PRESION/layer thickness , FORCE OF THE LITHOSPHERIC'/
     + 	   5X,' COLUMN OF DEPTH (layer thickness): ',F8.2,' km'/
     +  5X,'minima =',1P,E14.6,' N/m3,   maxima =',1P,E14.6,' N/m3'/)

C       WRITE(6,365) GLmin/1.D3,GLmax/1.D3,elevmin,elevmax,sedmin,sedmax,
C     +		     rommin,rommax,ZASTH/1.D3,Pmin,Pmax,vismin,vismax
C 365   FORMAT(/2X,'LITHOSPHERIC THICKNESS: minimum =',F9.4,' km,',
C     +		'   maximum =',F9.4,' km'/
C     +	2X,'ELEVATION: minima =',F9.2,' m,   maxima =',F9.2,' m'/
C     +	2X,'SEDIMENTS: minim =',F9.2,' m,    maxim =',F9.2,' m'/
C     +	2X,'DENSITY OF LITHOSPHERIC MANTLE: minima =',F9.2,
C     +	' km/m3,    maxima =',F9.2,' km/m3'/2X, 
C     + 'PRESION, FORCE OF THE LITHOSPHERIC COLUMN OF DEPTH: ',F8.2,' km'
C     +  /5X,'minima =',1P,E12.4,' N/m3,   maxima =',1P,E12.4,' N/m3'/
C     +	2X,'VISCOSITY: minima =',1P,E16.6,' Pa.s,   maxima =',1P,E16.6,
C     +	' Pa.s'/)
...
CC  vissup [Pa*s]  Imposed upper limit for calculated viscosity 'vis'. (~10^25 Pa*s)
CC  visinf [Pa*s]  Imposed lower limit for calculated viscosity 'vis'. (~10^22 Pa*s)
CC  average_Pressure [Pa=N/m2] Integral of (P(z)*dz)/thickness_layer 
CC			between 
CC                      the surface (z=0) and ZASTH, Where P(z) is the lithostatic 
CC                      pressure and ZASTH is the level of compensation.
CC			where thickness_layer=ZASTH+topography(+)
CC  u, v [m/s]	Arrays of velocity in x, y directions in the previous time step.
CC			Sorted from west to east and south to north.
CC			Returns the new calculated velocity.
...
C 558		WRITE(6,565) n,iy,1,0.0,0.0		
...
CC 560		WRITE(6,*) 0,iy,vel(kvel),vel(kvel+1)
...
C ----------------------------------------------------------------------
 ...
 61   FORMAT(
     +  4X,'Viscosity:             minimum:',1P,G12.4,' Pa.s, maximum:',
     +    	1P,G12.4,' Pa.s'/
     +	4X,'Effective strain rate: medium:',1P,G12.4,' s-1,   maximum:',
     +		1P,G12.4,' s-1'/
     +	4X,'Effective strain:      medium:',1P,G12.4,'        maximum:',
     +		1P,G12.4)
...
!!           TERME2=0.D0 ! Sist. coord. LAGRANGIA (seguint el material)
...
CC --------------------------------------------------------------------
 ...
 65   FORMAT(4X,'Thickness:',13X,'minim',F10.2,' m',10X,
     +        'maxim: ',F10.2,' m')
