...
float dist_xy_line(
	...
	float y, 		/* y     "    					*/
	float *lineX, 		/* Poligon x coordinates (lineX[np])		*/
	float *lineY, 		/* Poligon y coordinates (lineY[np])		*/
...

{
	...
	/*
	  RETURNS THE DISTANCE FROM X-Y POINT TO A POLIGON.
	  ATTENTION: returned distance is <0 negative! if the poligon is clockwise 
...
{
/*
		 CALCULATES GEOID HEIGHT ANOMALY DUE TO A POLIGON 2D 
	(3D HORIZONTAL INFINITE POLIEDRUS). INTERNATIONAL SYSTEM UNITS USED.

	Poligon must be clock-wise sorted, otherwise anomaly sign is changed.
...
float gravanompolig(
	float *x_pol, 				/*x-y points of poligon clock-wise sorted*/
	float *z_pol, 				/*x-y points of poligon clock-wise sorted. z>0 downwards*/
	int numpoints, 				/*Number of poligon points*/
...
/*
	...
	TO A POLIGON 2D (3D HORIZONTAL INFINITE POLIEDRUS). 
	International System units used.
	Poligon must be clock-wise sorted, otherwise anomaly sign is changed.
	z are positive vertical distances if the poligon is under measure point.
				...
				*/
{
	...
	int 	i, j, k, 
		closestdatinputnum, 
		npols, nmax_pols=20, *n_points_pol;
	...
	switch (mode_interp) {
	  ...
	  case 9:
	  case 10:
		/*Defines the poligons in variables*/
		...
		i-th poligon but outside the (i+1)th poligon then weights with 
...
{
	/*
		RETURNS 0 IF x,y IS OUTSIDE OF poligon, 1 IF INSIDE, -1 IF 
		BELONGING TO IT.
		'Inside' includes any finite closed region of 2D cartesian space, 
		(i.e., if the poligon cuts itself n times then n+1 closed 
		...
		load: total flux along a  closed poligon including the load is
		non-zero; whereas flux  along a poligon which does not include
		the load is zero.
	...
	*/
	...
	for (i=0; i<np; i++){
		x0 = poligonX[i];
		y0 = poligonY[i];
		if (i<(np-1)) xf = poligonX[i+1];  else xf = poligonX[0];
		if (i<(np-1)) yf = poligonY[i+1];  else yf = poligonY[0];

		D2 = (x0-x )*(x0-x ) + (y0-y )*(y0-y);
		E2 = (xf-x )*(xf-x ) + (yf-y )*(yf-y);	
		L2 = (xf-x0)*(xf-x0) + (yf-y0)*(yf-y0);
		L = sqrt(L2);					/*Length of this poligon side.*/
		...
		/*If the point coincides with the point of poligon:*/
		if (D == 0) return(-1);
		if (E == 0) return(-1);
		...
		/*If x,y is alineated with this poligon side:*/
		if (disc==0)  partial_circ =  + 1/D - 1/E;  
		/*If a point is repeated in the poligon:*/
		if (L == 0)   partial_circ = 0;

		circulation += partial_circ;
	}

	/*printf("\nLength of poligon = %.2f\nCirculation = %.5e\n", pol_length, circulation);*/
...
float poligon_area(
	float *X, 	/* Poligon x coordinates (poligonX[np])*/
	float *Y, 	/* Poligon y coordinates (poligonY[np])*/
	int np)		/* Number of points in poligon.*/

{
	/*
		RETURNS THE AREA OF A POLIGON. EVEN NON CONVEX POLIGONS.
		Positive area if the poligon is given clockwise; negative 
		otherwise.
		You dont need to close the poligon.
